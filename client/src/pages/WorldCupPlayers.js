...
export default function WorldCupPlayers() {
  ...
  useEffect(() => {
    fetch(`http://${config.server_host}:${config.server_port}/get_world_cup_players`)
      .then(res => res.json())
      .then(resJson => {
  ...
  const search = () => {
    ...
    fetch(`http://${config.server_host}:${config.server_port}/get_world_cup_players?player_name=${searchPlayerName}` +
  ...
  return (
    <Container>
      <h2>Search World Cup Players</h2>
